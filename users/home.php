...
$sql = "select * from $tbl where t_status=0";     
...
?>
                                    <table id="my-table">
                                        <thead>
                                            <tr>
                                            <?php
                                            $result_colu = mysqli_query($conn, $sql_columns);

                                            if (!$result_colu) {
                                                die("Query failed: " . mysqli_error($conn));
                                            }
                                            while ($colu = mysqli_fetch_assoc($result_colu)) {
                                                echo "<th>" . ucfirst($colu['Field']) . "</th>";
                                            } 
                                            echo"</tr></thead><tbody>";
                                            // Query to fetch the data dynamically
                                            $sql_data = "SELECT * FROM $tbl WHERE t_status=1";
                                            ...
                                            if (mysqli_num_rows($result_data) > 0) {
                                                ...
                                                echo "<tr><td colspan='" . mysqli_num_fields($result_data) . "'>No data available</td></tr>";
                                            }
...
const filename = `<?php echo $tbl?>${year}-${month}-${day}_${hours}-${minutes}-${seconds}.xlsx`;
